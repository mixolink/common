...
public class PerformanceCalculator {
	...
	public static enum DisplayPerformanceFormat {
		ALL, SIZE, COUNT;// , BITS_PER_SECOND
	}

	public static String COUNT_UNIT = "count/s";

	public PerformanceCalculator() {
	}

	public static String calcPerformance(DisplayPerformanceFormat sp, long snapshotTime, CountSizeStatistic a, CountSizeStatistic b) {
		long endT = System.currentTimeMillis();
		return calcPerformance(sp, snapshotTime, endT, a, b);
	}

//	public static int calcRemainSecond(DisplayPerformanceFormat sp, int usedSecond, FileStatistics total, FileStatistics current) {
//
//		if (usedSecond <= 0) {
//			usedSecond = 1;
//		}
...
//	}
	 ...
	 * @return
	 */
	public static int calcRemainSecond(DisplayPerformanceFormat sp, double usedSecond, CountSizeStatistic total, CountSizeStatistic current) {
...
//	public static double calcAveragePerformance(DisplayPerformanceFormat sp, int usedSecond, FileStatistics total, FileStatistics current) {
//
//		if (usedSecond <= 0) {
//			usedSecond = 1;
//		}
...
//	}
	...
	public static String calcPerformance(DisplayPerformanceFormat sp, long snapshotTime, long endTime, CountSizeStatistic a, CountSizeStatistic b) {
		if (a == null || b == null) {
			return "";
		}
		...
		case ALL: {
			...
			performance = FormatUtils.getPrintSize(perBytesSec, false) + "/s | " + FormatUtils.formatNumber(countPerSec) + " " + COUNT_UNIT;
//			performance = FormatUtils.getPrintBps(perBytesSec) + " | " + FormatUtils.getPrintSize(perBytesSec, false) + "/s | "
//					+ FormatUtils.formatNumber(countPerSec) + " count/s";
			break;
		}
//		case BITS_PER_SECOND: {
//			long totalSize = Math.abs(a.getTotalSize() - b.getTotalSize());
...
//		}
		...
		case COUNT: {
			double totalCount = Math.abs(a.getTotalCount() - b.getTotalCount());
			int countPerSec = (int) (totalCount / usedSecond);
			performance = FormatUtils.formatNumber(countPerSec) + " " + COUNT_UNIT;
			break;
		}
		}
	...
	public static String calcPerformance(DisplayPerformanceFormat sp, long usedMS, long value) {
		if (value <= 0) {
			return "";
		}

		// to second
		double usedSecond = Math.abs(usedMS) / 1000D;
...
//		}
		case SIZE: {
			long perBytesSec = (long) (value / usedSecond);
			performance = FormatUtils.getPrintSize(perBytesSec, false) + "/s";
			break;
		}
		case COUNT: {
			int countPerSec = (int) (value / usedSecond);
			performance = FormatUtils.formatNumber(countPerSec) + " " + COUNT_UNIT;
			break;
		}
		}
