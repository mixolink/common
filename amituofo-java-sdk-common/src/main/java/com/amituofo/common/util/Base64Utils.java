...
public class Base64Utils {
	...
	/*
	 * The methods of this class are static. Do not instantiate this class. Use its static methods to get the encoded/decoded results
	 */
	...
	public static String encode(byte[] byteData, String encoding) {
		...
		try {
			...
			return new String(_encode(byteData));
		}
	}
	...
	public static byte[] encode(String string, String encoding) {
		...
		try {
			...
			return _encode(string.getBytes());
		}
	}
	...
	public final static byte[] _encode(byte[] byteData) {
		...
		/*
		 * Declare working variables including an array of bytes that will contain the encoded data to be returned to the caller. Note that the
		 * encoded array is about 1/3 larger than the input. This is because every group of 3 bytes is being encoded into 4 bytes.
		 */
		...
		/*
		 * Walk through the input array, 24 bits at a time, converting them from 3 groups of 8 to 4 groups of 6 with two unset bits between. as per
		 * Base64 spec see http://www.javaworld.com/javaworld/javatips/jw-javatip36-p2.html for example explanation
		 */
	...
	public static String decode(byte[] encoded, String encoding) {
		...
		try {
			...
			return new String(_decode(encoded));
		}
	}
	...
	public final static byte[] decode(String encoded, String encoding) {
		...
		try {
			...
			return _decode(encoded.getBytes());
		}
	}
	...
	public final static byte[] _decode(byte[] byteData) {
		...
		/*
		 * Declare working variables including an array of bytes that will contain the decoded data to be returned to the caller. Note that the
		 * decoded array is about 3/4 smaller than the input. This is because every group of 4 bytes is being encoded into 3 bytes.
		 */
		int iSrcIdx; // index into source (byteData)
		...
		/*
		 * remove any '=' chars from the end of the byteData they would have been padding to make it up to groups of 4 bytes note that I don't need
		 * to remove it just make sure that when progressing throug array we don't go past reviSrcIdx ;-)
		 */
		...
		/*
		 * Convert from Base64 alphabet to encoded data (The Base64 alphabet is completely documented in RFC 1521.) The order of the testing is
		 * important as I use the '<' operator which looks at the hex value of these ASCII chars. So convert from the smallest up
		 *
		...
		/*
		 * 4bytes -> 3bytes munge Walk through the input array, 32 bits at a time, converting them from 4 groups of 6 to 3 groups of 8 removing the
		 * two unset most significant bits of each sorce byte as this was filler, as per Base64 spec. stop before potential buffer overun on
		 * byteDest, remember that byteDest is 3/4 (integer division) the size of input and won't necessary divide exactly (ie iDestIdx must be <
		 * (integer div byteDest.length / 3)*3 see http://www.javaworld.com/javaworld/javatips/jw-javatip36-p2.html for example
		 */
		...
		/*
		 * tidy up any remainders if iDestIdx >= ((byteDest.length / 3)*3) but iSrcIdx < reviSrcIdx then we have at most 2 extra destination bytes
		...
		if (iSrcIdx < reviSrcIdx) {
			...
			/*
			 * wont have just one input byte left (unless input wasn't base64 encoded ) due to the for loop steps and array sizes, after "=" pad
			 * removed, but for compleatness
			 */
			else {
