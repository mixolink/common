 ...
 * @version 2.0
 */
public class DropActionOverlay0 {
	 ...
	 */
	public DropActionOverlay0(JComponent targetComponent, JLayeredPane layeredPane) {
	...
	private void setupDropTarget() {
		new DropTarget(targetComponent, new UniversalDropTargetListener());
		new DropTarget(layeredPane, new UniversalDropTargetListener());
	}
	 ...
	 * 通用拖放监听器
	 */
	private class UniversalDropTargetListener extends DropTargetAdapter {

		@Override
		public void dragEnter(DropTargetDragEvent dtde) {
			System.out.println(
					"✅ dragEnter: " + dtde.getLocation() + " | Component: " + dtde.getDropTargetContext().getComponent().hashCode() + " | Component: " + dtde.getSource().hashCode());

			if (isDragAcceptable(dtde)) {
				if (enableOverlay && !dropActions.isEmpty()) {
					overlayPanel.setVisible(true);
					overlayPanel.setDragPosition(dtde.getLocation());
		...
		public void dragOver(DropTargetDragEvent dtde) {
			System.out.println(
					"🔄 dragOver: " + dtde.getLocation() + " | Component: " + dtde.getDropTargetContext().getComponent().hashCode() + " | Component: " + dtde.getSource().hashCode());
		...
		public void dragExit(DropTargetEvent dte) {
			if (overlayPanel.isVisible()) {
				System.out.println("❌ dragExit called! Source: " + dte.getSource());
				if (enableOverlay) {
					overlayPanel.setVisible(false);
				}
			}
		...
		public void drop(DropTargetDropEvent dtde) {
			try {
				if (isDragAcceptable(dtde)) {
					...
					DropAction action;
					if (enableOverlay && overlayPanel.isVisible()) {
						action = overlayPanel.getSelectedAction(dtde.getLocation());
						if (action == null) {
...
//					Transferable transferable = dtde.getTransferable();
//					if (transferable != null && transferable instanceof ComponentTransferable) {
//						if (dtde.getDropTargetContext().getComponent() == ((ComponentTransferable) transferable).getSourceComponent()) {
//							return false;
//						}
//					}
					return true;
		...
		private boolean isDragAcceptable(DropTargetDropEvent dtde) {
			for (DataFlavor flavor : acceptedFlavors) {
				if (dtde.isDataFlavorSupported(flavor)) {
					return true;
				}
		...
		private DataFlavor getAcceptedFlavor(Transferable t) {
			for (DataFlavor flavor : acceptedFlavors) {
	 ...
	 * 覆盖面板
	 */
	class DropOverlayPanel extends JPanel {
